...
class LevelEighteen extends Phaser.Scene {
	...
	editorCreate() {
		...
		// rectangle_24
		const rectangle_24 = this.add.rectangle(600, 460, 10, 200);
	...
	create() {
		...
		let tempEndTime = Date.now() + this.oGameManager.levelEighteenTimer;
	...
	startLevelTimer() {
		if(!this.isTimerRunning && this.isTimerStarted) {
			this.endTime = Date.now() + this.oGameManager.levelEighteenTimer;
