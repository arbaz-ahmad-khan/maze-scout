...
class LevelSixteen extends Phaser.Scene {
	...
	editorCreate() {
		...
		// rectangle_30
		const rectangle_30 = this.add.rectangle(490, 311, 10, 100);
	...
	create() {
		...
		let tempEndTime = Date.now() + this.oGameManager.levelSixteenTimer;
	...
	startLevelTimer() {
		if(!this.isTimerRunning && this.isTimerStarted) {
			this.endTime = Date.now() + this.oGameManager.levelSixteenTimer;
